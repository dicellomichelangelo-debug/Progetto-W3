...
*/
me.skills=['HTML','CSS','Javascript','C++','Java']
// console.log(me)
...
// console.log(onlyLetters('salve, io ho 4 cani'))
...
// console.log(isThisAnEmail('giorgiovanni@gmail.com'))
...
// console.log(deleteProp(test, 'età'))
const movies = [
  {
...
const newestMovie= function(){
    let mostRec= movies[0]
    for(let i=1; i<movies.length;i++){
        if(movies[i].Year>mostRec.Year){
            mostRec= movies[i]
        }
    }
    return mostRec
...
// console.log('il numero dei film presenti in movies è '+ countMovies())
...
const searchAndDivide= function(str){
      let corrispondenza=[]
      let nonCorrispondenza=[]
      for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.toLowerCase().includes(str.toLowerCase())) {
      corrispondenza.push(movies[i]);
    } else {
      nonCorrispondenza.push(movies[i]);
    }
  }
    return{
        Trovato: corrispondenza,
        nonTrovato: nonCorrispondenza
    }
}
